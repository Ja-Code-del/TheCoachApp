...
export default function JourJScreen({ eventName, theme, onShare, isSharing, onAddEvent, onDismiss }) {
  ...
  const centerSlide    = useRef(new Animated.Value(18)).current;
  const actionsOpacity = useRef(new Animated.Value(0)).current;
  const actionsSlide   = useRef(new Animated.Value(14)).current;
  const btnOpacity     = useRef(new Animated.Value(0)).current;
  const btnSlide       = useRef(new Animated.Value(10)).current;
  ...
  useEffect(() => {
    Animated.sequence([
      ...
      Animated.parallel([
        Animated.timing(topOpacity, { toValue: 1, duration: 300, useNativeDriver: true }),
        Animated.spring(topSlide,   { toValue: 0, friction: 10, tension: 60, useNativeDriver: true }),
      ]),
      Animated.parallel([
        Animated.timing(centerOpacity, { toValue: 1, duration: 380, useNativeDriver: true }),
        ...
        Animated.timing(starOpacity,   { toValue: 1, duration: 380, useNativeDriver: true }),
        Animated.timing(starRotate,    { toValue: 1, duration: 560, useNativeDriver: true }),
      ]),
      Animated.parallel([
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Animated.View style={[
          ...
          {/* Ajouter */}
          <TouchableOpacity style={styles.actionBtn} onPress={onAddEvent} activeOpacity={0.75}>
            <View style={styles.actionCircle}>
              <Feather name="plus" size={26} color="rgba(255,255,255,0.9)" />
            </View>
            <Text style={[styles.actionLabel, { fontFamily: 'Inter_700Bold' }]}>Ajouter</Text>
            <Text style={[styles.actionSub, { fontFamily: 'Inter_300Light' }]}>Nouvel event</Text>
          </TouchableOpacity>

          {/* Suivant */}
          <TouchableOpacity style={styles.actionBtn} onPress={onDismiss} activeOpacity={0.75}>
            <View style={styles.actionCircle}>
              <Feather name="chevron-right" size={26} color="rgba(255,255,255,0.9)" />
            </View>
            <Text style={[styles.actionLabel, { fontFamily: 'Inter_700Bold' }]}>Suivant</Text>
            <Text style={[styles.actionSub, { fontFamily: 'Inter_300Light' }]}>Event suivant</Text>
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  // Layout — 4 zones avec respiration naturelle
  content: {
    position: 'absolute',
  ...
  // Actions — cercles compacts avec label + sous-titre
  actionsBlock: {
    flexDirection: 'row',
  ...
  shareBtnText: { color: '#111', fontSize: 15 },
});