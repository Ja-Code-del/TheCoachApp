...
// ─── TIME PICKER MODAL ───────────────────────────────────────────────────────
function TimePickerModal({ visible, currentTime, onConfirm, onClear, onClose }) {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.field}>
          ...
          {activeEvent.targetTime && (
            <Text style={[styles.hint, { fontFamily: 'Inter_300Light' }]}>
              ⏱ Décompte précis jusqu'à {activeEvent.targetTime} le jour J.
            </Text>
          )}
        </View>
      ...
      <DatePickerModal
        ...
        onConfirm={(d) => { onUpdateEvent({ targetDate: dateToStr(d) }); setShowDateModal(false); }}
        onClose={() => setShowDateModal(false)}
      />
      <TimePickerModal
        visible={showTimeModal}
        currentTime={activeEvent.targetTime ?? null}
        onConfirm={(time) => { onUpdateEvent({ targetTime: time }); setShowTimeModal(false); }}
        onClear={() => { onUpdateEvent({ targetTime: null }); setShowTimeModal(false); }}
