  ...
  return (
    <div className="relative z-10 px-8 pt-6 pb-6 flex flex-col gap-3 items-center text-center" style={{ height: '647px' }}>
      ...
      <div className="w-full flex justify-between items-center flex-shrink-0">
        <Tooltip label={t('tooltips.addEvent')}>
          <button title={t('tooltips.addEvent')} onClick={onAddEvent} className="p-3 hover:bg-white/10 rounded-full transition-colors">
            <FontAwesomeIcon icon={faPlus} className="text-xl opacity-70" />
          </button>
        </Tooltip>
        <div className="flex items-center gap-1">
          <Tooltip label={t('tooltips.changeImage')}>
            <button title={t('tooltips.changeImage')} onClick={onRefreshImage} disabled={isLoadingImage || !activeEvent.theme}
          ...
          <Tooltip label={t('tooltips.settings')}>
            <button title={t('tooltips.settings')} onClick={onOpenSettings} className="p-3 hover:bg-white/10 rounded-full transition-colors">
      ...
      <div className="w-full bg-black/20 backdrop-blur-md px-5 py-4 rounded-[2rem] border border-white/10 shadow-inner flex-1 min-h-0 flex flex-col">
        ...
        <Tooltip label={t('tooltips.newQuote')}>
          <button title={t('tooltips.newQuote')} onClick={onRefreshQuote} disabled={isLoadingQuote}
      ...
      <div className="w-full flex flex-col items-center gap-2 flex-shrink-0">
        <button title={t('display.share')} onClick={onShare} disabled={isSharing}
